...
{
  TRACE_F("rc: %p, id: '%s', v: %p, n: %" rdint_fmt ", inc: %" rdint_fmt, rc, id, v, n, inc);
...
#if REDIRECT_CALL_VECTOR_DENSE
  if (inc == 1)
  {
    redirect_call_put_input_param_dense_val<T>(rc, id, v, n);
...
void redirect_call_get_input_param_vector_val(redirect_call_t *rc, const char *id, T **v, rdint_t *n, rdint_t *inc)
{
  ...
  *n = x[0];
  /* the sender inc in x[1] is only used to select dense or blocks */

#if REDIRECT_CALL_VECTOR_DENSE
  if (x[1] == 1)
  {
    redirect_call_get_input_param_dense_val<T>(rc, id, v, *n);
    *inc = 1;
...
void redirect_call_put_output_param_vector_val(redirect_call_t *rc, const char *id, const T *v, rdint_t n, rdint_t inc)
{
  ...
  const rdint_t x[2] = { n, inc };
  redirect_call_put_output_conf_rdints(rc, "v", x, 2);

#if REDIRECT_CALL_VECTOR_DENSE
  if (inc == 1)
  {
    redirect_call_put_output_param_dense_val<T>(rc, id, v, n);
...
void redirect_call_get_output_param_vector_val(redirect_call_t *rc, const char *id, T **v, rdint_t *n, rdint_t *inc)
{
  ...
  rdint_t x[2];
  redirect_call_get_output_conf_rdints(rc, "v", x, 2);
  *n = x[0];
  /* the sender inc in x[1] is only used to select dense or blocks */
  ...
  if (x[1] == 1)
  {
    redirect_call_get_output_param_dense_val<T>(rc, id, v, *n);
    *inc = 1;
...
void redirect_call_put_inout_param_vector_val(redirect_call_t *rc, const char *id, const T *v, rdint_t n, rdint_t inc)
{
  ...
  if (inc == 1)
  {
    redirect_call_put_inout_param_dense_val<T>(rc, id, v, n);
...
void redirect_call_get_inout_param_vector_val(redirect_call_t *rc, const char *id, T **v, rdint_t *n, rdint_t *inc)
{
  ...
  *n = x[0];
  ...
  if (x[1] == 1)
  {
    redirect_call_get_inout_param_dense_val<T>(rc, id, v, *n);
    *inc = 1;
