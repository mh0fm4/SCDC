...
static void do_fcs_init(fcs_call_t *fc)
{
  ...
  if (return_code != FCS_SUCCESS)
  {
    ...
    FCS_CALL(set_result_str)(fc, message);
...
#if LIBFCS_SCDC_CACHE
  fcs_cache_t *fch = (fcs_cache_t *) (fc + 1);
...
scdcint_t fcs_scdc_dataset_cmd(void *dataprov, void *dataset, const char *cmd, const char *params, scdc_dataset_input_t *input, scdc_dataset_output_t *output, scdc_result_t *result)
{
  ...
  FCS_CALL(init_scdc)(fc);

#if LIBFCS_SCDC_CACHE
  fcs_cache_t *fch = (fcs_cache_t *) (fc + 1);

  FCS_CALL(set_cache_ptr)(fc, fch);
#endif

  if (FCS_CALL(cmd)(fc, cmd, params, input, output, result))
  {
