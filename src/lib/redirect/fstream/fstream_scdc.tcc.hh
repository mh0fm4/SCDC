...
#ifndef __FSTREAM_SCDC_TCC__
#define __FSTREAM_SCDC_TCC__
...
#define __class__  "basic_filebuf_scdc"
...
#if USE_FSTREAM_SCDC
    _M_file_type_scdc(false), _M_file_scdc(),
#endif /* USE_FSTREAM_SCDC */
...
#if USE_FSTREAM_SCDC
    _M_file_type_scdc(__rhs._M_file_type_scdc), _M_file_scdc(),
#endif /* USE_FSTREAM_SCDC */
...
#if USE_FSTREAM_SCDC
      _M_file_type_scdc = std::__exchange(__rhs._M_file_type_scdc, false);
      _M_file_scdc.swap(__rhs._M_file_scdc);
#endif /* USE_FSTREAM_SCDC */
      _M_mode = std::__exchange(__rhs._M_mode, ios_base::openmode(0));
      _M_state_beg = std::move(__rhs._M_state_beg);
      _M_state_cur = std::move(__rhs._M_state_cur);
  ...
  template<typename _CharT, typename _Traits>
    ...
    swap(basic_filebuf& __rhs)
    {
      ...
      _M_file.swap(__rhs._M_file);
#if USE_FSTREAM_SCDC
      std::swap(_M_file_type_scdc, __rhs._M_file_type_scdc);
      _M_file_scdc.swap(__rhs._M_file_scdc);
#endif /* USE_FSTREAM_SCDC */
  ...
  template<typename _CharT, typename _Traits>
	...
	{
#if USE_FSTREAM_SCDC
    if (__file_type_scdc::is_valid(__s))
    {
      _M_file_type_scdc = true;
      _M_file_scdc.open(__s, __mode);
    } else
#endif
    {
  ...
  template<typename _CharT, typename _Traits>
	...
	__catch(...)
      ...
      if (_M_file_type_scdc)
      {
        if (!_M_file_scdc.close()) __testfail = true;
      } else
#endif /* USE_FSTREAM_SCDC */
      {
      if (!_M_file.close())
  ...
  //showmanyc: pronounced "ees-how-many-see"
  //Returns an estimation of the number of characters that are at least available for input or -1.
  //If the returned value is not -1, the indicated number of characters can be made available by one or more calls
  //to underflow(). If this returns -1 underflow() will fail. 
  //Purpose is the know that underflow() will return "immmediately" or if the extern devcice can be read without waiting
  //and that it not returns eof.
  //This is highly device-specific
  //It's OK to return 0 (basic_streambuf's implementation always returns 0) 
  //or don't implemet this if you have no idea how to implement it.
  //
  //(source: Standard C++ IOStreams and Locales, A. Langer and K. Kreft)
  template<typename _CharT, typename _Traits>
    streamsize
    basic_filebuf<_CharT, _Traits>::
    showmanyc()
...
#endif
    {
      ...
      if (_M_file_type_scdc)
      {
	       __ret += _M_file_scdc.showmanyc() / _M_codecvt->max_length();
      } else
#endif /* USE_FSTREAM_SCDC */
      {
  ...
  template<typename _CharT, typename _Traits>
      ...
      if (__testin)
	{
	  ...
	  if (__check_facet(_M_codecvt).always_noconv())
        ...
        if (_M_file_type_scdc)
        {
	        __ilen = _M_file_scdc.xsgetn(reinterpret_cast<char*>(this->eback()), __buflen);
        } else
#endif /* USE_FSTREAM_SCDC */
        {
	      ...
	      do
		{
          ...
          if (_M_file_type_scdc)
          {
		          __elen = _M_file_scdc.xsgetn(_M_ext_end, __rlen);
          } else
#endif /* USE_FSTREAM_SCDC */
          {
		      __elen = _M_file.xsgetn(_M_ext_end, __rlen);
          }
		      if (__elen == 0)
			__got_eof = true;
		      else if (__elen == -1)
  ...
  template<typename _CharT, typename _Traits>
    ...
    if (_M_file_type_scdc)
    {
      __elen = _M_file_scdc.xsputn(reinterpret_cast<char*>(__ibuf), __ilen);
    } else
#endif /* USE_FSTREAM_SCDC */
    {
    ...
    if (_M_file_type_scdc)
    {
      __elen = _M_file_scdc.xsputn(__buf, __blen);
    } else
#endif /* USE_FSTREAM_SCDC */
    {
      ...
      if (_M_file_type_scdc)
      {
        __elen = _M_file_scdc.xsputn(__buf, __rlen);
      } else
#endif /* USE_FSTREAM_SCDC */
      {
      ...
      else if (_M_writing)
	{
 	   ...
 	   && __testin)
 	 {
 	   ...
 	   for (;;)
         ...
         if (_M_file_type_scdc)
         {
           __len = _M_file_scdc.xsgetn(reinterpret_cast<char*> (__s), __n);
         } else
#endif /* USE_FSTREAM_SCDC */
         {
  ...
  template<typename _CharT, typename _Traits>
 	   ...
 	   && __testout && !_M_reading)
	{
	  ...
	  if (__n >= __limit)
        ...
        if (_M_file_type_scdc)
        {
          __ret = _M_file_scdc.xsputn_2(__buf, __buffill, reinterpret_cast<const char*>(__s), __n);
        } else
#endif /* USE_FSTREAM_SCDC */
        {
  ...
  template<typename _CharT, typename _Traits>
      ...
      if (this->is_open() && !__testfail)
	{
	  ...
	  if (!__no_movement)
        ...
        if (_M_file_type_scdc)
        {
          __file_off = _M_file_scdc.seekoff(0, ios_base::cur);
        } else
#endif /* USE_FSTREAM_SCDC */
        {
  ...
  off_type __file_off;
    ...
    if (_M_file_type_scdc)
    {
      __file_off = _M_file_scdc.seekoff(__off, __way);
    } else
#endif /* USE_FSTREAM_SCDC */
    {
	  __file_off = _M_file.seekoff(__off, __way);
    }
	  if (__file_off != off_type(-1))
	    {
	      _M_reading = false;
	  ...
	  && __testvalid)
	{
	  ...
	  do
		       ...
		       __r == codecvt_base::partial)
		{
          ...
          if (_M_file_type_scdc)
          {
            __elen = _M_file_scdc.xsputn(__buf, __ilen);
          } else
#endif /* USE_FSTREAM_SCDC */
          {
		      __elen = _M_file.xsputn(__buf, __ilen);
          }
		      if (__elen != __ilen)
			__testvalid = false;
		    }
  ...
  // Disabled because there are no explicit instantiations elsewhere.
#if 0
