 ...
 */
class scdc_dataprov_webdav2_response_data {
public:
    ...
    //
    //    scdc_dataprov_webdav2_response_data()
...
protected:
     ...
     */
    scdc_dataprov_webdav2_response_data* response_data;

public:

    virtual ~scdc_dataprov_webdav2_session_handler(){};
    ...
    */
     ...
     * If an error occurred this method returns false and a description of the error is saved in the scdc_dataprov_webdav2_response_data.
     * @return true on success, false otherwise
     * @see scdc_dataprov_webdav2_response_data
     */
    ...
    virtual scdc_dataprov_webdav2_response_data* get_response_data() const {
    ...
    virtual void set_response_data(scdc_dataprov_webdav2_response_data* response_data) {
        this->response_data = response_data;
    }
};


bool scdc_dataprov_webdav2_session_handler::set_session_handler_config(const string & config) {
 ...
 */
class scdc_dataprov_webdav2_session_handler_davix : public scdc_dataprov_webdav2_session_handler {
private:
...
public:
    ...
    scdc_dataprov_webdav2_session_handler_davix();
    ~scdc_dataprov_webdav2_session_handler_davix();
...
scdc_dataprov_webdav2_session_handler_davix::scdc_dataprov_webdav2_session_handler_davix() {
    base_path = "/";
    posix_open_mode = -1;
    dav_fd = NULL;
    response_data = new scdc_dataprov_webdav2_response_data();
}
...
void scdc_dataprov_webdav2_session_handler_davix::set_username_password(const string username, const string password) {
    request_parameters.setClientLoginPassword(username, password);
}

Uri scdc_dataprov_webdav2_session_handler_davix::get_davix_uri(string path) {
    return Uri(protocol + "://" + host + base_path + path);
}

void scdc_dataprov_webdav2_session_handler_davix::set_error_to_response_data(DavixError* err) {
    set_error_to_response_data(err->getErrScope(), err->getErrMsg());
    err->clearError(&err);
}

void scdc_dataprov_webdav2_session_handler_davix::set_error_to_response_data(string scope, string msg) {
    delete response_data;
    response_data = new scdc_dataprov_webdav2_response_data();
    response_data->error_message = "Scope: " + scope + " | Error message: " + msg;
}

void scdc_dataprov_webdav2_session_handler_davix::set_base_path(string path) {
...
bool scdc_dataprov_webdav2_session_handler_davix::new_session(string protocol, string host, string base_path, string username, string password) {
    this->protocol = protocol;
    this->host = host;
    this->base_path = base_path;
...
bool scdc_dataprov_webdav2_session_handler_davix::new_session() {
...
bool scdc_dataprov_webdav2_session_handler_davix::check_configuration() {
    struct stat info;
    return get_stat("", &info); //read info from the base path
}

bool scdc_dataprov_webdav2_session_handler_davix::open_file(string path, int open_mode) {
...
string scdc_dataprov_webdav2_session_handler_davix::get_open_mode_as_string(int open_mode) {
...
bool scdc_dataprov_webdav2_session_handler_davix::open_file_with_davix(string path, int open_mode) {
    DavixError* err = NULL;
...
bool scdc_dataprov_webdav2_session_handler_davix::open_file_to_write_trunc(string path, int open_mode) {
...
bool scdc_dataprov_webdav2_session_handler_davix::open_file_to_write_append(string path, int open_mode) {
...
bool scdc_dataprov_webdav2_session_handler_davix::close_file() {
...
bool scdc_dataprov_webdav2_session_handler_davix::get_file_content(string path, vector<char> *buffer) {
    DavixError* err = NULL;
...
scdcint_t scdc_dataprov_webdav2_session_handler_davix::read_into_buffer(void* buffer, scdcint_t buffer_size) {
    DavixError* err = NULL;
...
scdcint_t scdc_dataprov_webdav2_session_handler_davix::lseek(int flag, scdcint_t offset) {
    DavixError* err = NULL;
...
scdcint_t scdc_dataprov_webdav2_session_handler_davix::write_buffer_to_file(const void* buffer, scdcint_t count) {
...
bool scdc_dataprov_webdav2_session_handler_davix::remove_file(string path) {
    DavixError* err = NULL;
...
bool scdc_dataprov_webdav2_session_handler_davix::is_dir(string path) {
...
string scdc_dataprov_webdav2_session_handler_davix::read_dir(string path) {
...
bool scdc_dataprov_webdav2_session_handler_davix::get_stat(string path, struct stat * info) {
    DavixError* err = NULL;
    DavPosix pos(&context);
 ...
 *
 */
class scdc_dataprov_webdav2_session_handler_factory {
public:
     ...
     */
    static scdc_dataprov_webdav2_session_handler* get_new_session_hander() {
        return new scdc_dataprov_webdav2_session_handler_davix();
    }
    ...
    //        return new scdc_dataprov_webdav2_session_handler_davix();
    //#elif USE_NEON
    //        return new scdc_dataprov_webdav2_session_handler_neon();
    //#else
...
class scdc_dataset_webdav2_access : public scdc_dataset {
public:

    scdc_dataset_webdav2_access(scdc_dataprov *dataprov_) : scdc_dataset(dataprov_) {
    };
     ...
     * @return true on success and false if an error occurred
     */
    bool do_cmd_lseek(const string &params, scdc_dataset_input_t *input, scdc_dataset_output_t *output, scdc_result &result);
     ...
     * @return true on success and false if an error occurred
     */
    bool do_cmd_open(const string &params, scdc_dataset_input_t *input, scdc_dataset_output_t *output, scdc_result &result);
     ...
     * @return true on success and false if an error occurred
     */
    bool do_cmd_close(const string &params, scdc_dataset_input_t *input, scdc_dataset_output_t *output, scdc_result &result);
...
private:
     ...
     * @param output output struct
     */
    static void set_response_error_to_inout(scdc_dataprov_webdav2_session_handler *session_handler, scdc_dataset_inout_t *output);
     ...
     */
    static scdcint_t dataset_webdav2_read_file_next(scdc_dataset_inout_t *inout, scdc_result_t *result);
};

bool scdc_dataset_webdav2_access::do_cmd_info(const string& params, scdc_dataset_input_t* input, scdc_dataset_output_t * output, scdc_result &result) {
    ...
    scdc_dataprov_webdav2_access *dataprov_webdav = static_cast<scdc_dataprov_webdav2_access *> (dataprov);
...
bool scdc_dataset_webdav2_access::do_cmd_cd(const string& params, scdc_dataset_input_t* input, scdc_dataset_output_t * output, scdc_result &result) {
...
bool scdc_dataset_webdav2_access::do_cmd_ls(const string& params, scdc_dataset_input_t* input, scdc_dataset_output_t * output, scdc_result &result) {
    ...
    string res = dataprov_webdav->session_handler->read_dir(params);

    if (!res.empty()) {
        if (static_cast<size_t>(SCDC_DATASET_INOUT_BUF_SIZE(output)) < res.size()) { //FIXEME implement next function for output for do_cmd_ls
            SCDC_ERROR("do_cmd_ls: the given buffer is to small to hold all the information.");
        }
        set_string_result_to_inout(res, output);
        return SCDC_SUCCESS;
    } else {
        set_response_error_to_inout(dataprov_webdav->session_handler, output);
        return SCDC_FAILURE;
...
bool scdc_dataset_webdav2_access::do_cmd_put(const string& params, scdc_dataset_input_t* input, scdc_dataset_output_t * output, scdc_result &result) {
...
bool scdc_dataset_webdav2_access::do_cmd_get(const string& params, scdc_dataset_input_t* input, scdc_dataset_output_t * output, scdc_result &result) {
    ...
    output->data = static_cast<scdc_dataprov_webdav2_access *> (dataprov);
    output->next = dataset_webdav2_read_file_next;

    //request the first data from the given file
    if (output->next(output, 0)) { //check for error
        return SCDC_SUCCESS;
    } else {
        set_response_error_to_inout(dataprov_webdav->session_handler, output);
        return SCDC_FAILURE;
...
scdcint_t scdc_dataset_webdav2_access::dataset_webdav2_read_file_next(scdc_dataset_inout_t * inout, scdc_result_t *result) {
    ...
    if (inout->data == NULL) {
        set_string_result_to_inout("dataset_webdav2_read_file_next: the data attribute was NULL (should be a pointer to a scdc_dataprov_webdav2_access object)", inout);
        return SCDC_FAILURE;
    }

    scdc_dataprov_webdav2_access *dataprov_webdav = static_cast<scdc_dataprov_webdav2_access *> (inout->data);
...
bool scdc_dataset_webdav2_access::do_cmd_rm(const string& params, scdc_dataset_input_t* input, scdc_dataset_output_t * output, scdc_result &result) {
...
bool scdc_dataset_webdav2_access::do_cmd_lseek(const string& params, scdc_dataset_input_t* input, scdc_dataset_output_t * output, scdc_result &result) {
    ...
    scdcint_t res = dataprov_webdav->session_handler->lseek(flag, offset);
    ...
    if (res == -1) {
        ...
        if (output != NULL && SCDC_DATASET_INOUT_BUF_PTR(output) != NULL && static_cast<size_t>(SCDC_DATASET_INOUT_BUF_SIZE(output)) >= sizeof (res)) {
...
bool scdc_dataset_webdav2_access::do_cmd_close(const string& params, scdc_dataset_input_t* input, scdc_dataset_output_t * output, scdc_result &result) {
...
bool scdc_dataset_webdav2_access::do_cmd_open(const string& params, scdc_dataset_input_t* input, scdc_dataset_output_t * output, scdc_result &result) {
...
void scdc_dataset_webdav2_access::set_string_result_to_inout(string result, scdc_dataset_inout_t * inout) {
...
void scdc_dataset_webdav2_access::set_response_error_to_inout(scdc_dataprov_webdav2_session_handler *session_handler, scdc_dataset_inout_t * inout) {
...
scdc_dataprov_webdav2_access::scdc_dataprov_webdav2_access()
: scdc_dataprov("webdav") {

    this->session_handler = scdc_dataprov_webdav2_session_handler_factory::get_new_session_hander();
}
...
bool scdc_dataprov_webdav2_access::open(const char *conf, scdc_args * args, scdc_result &result) {
    ...
    if (!scdc_dataprov::open(conf, args, result)) {
        ...
        //dataset_cmds_add("pwd", static_cast<dataset_cmds_do_cmd_f> (&scdc_dataset_webdav2_access::do_cmd_pwd));
        dataset_cmds_add("info", static_cast<dataset_cmds_do_cmd_f> (&scdc_dataset_webdav2_access::do_cmd_info));
        dataset_cmds_add("cd", static_cast<dataset_cmds_do_cmd_f> (&scdc_dataset_webdav2_access::do_cmd_cd));
        dataset_cmds_add("ls", static_cast<dataset_cmds_do_cmd_f> (&scdc_dataset_webdav2_access::do_cmd_ls));
        dataset_cmds_add("put", static_cast<dataset_cmds_do_cmd_f> (&scdc_dataset_webdav2_access::do_cmd_put));
        dataset_cmds_add("get", static_cast<dataset_cmds_do_cmd_f> (&scdc_dataset_webdav2_access::do_cmd_get));
        dataset_cmds_add("rm", static_cast<dataset_cmds_do_cmd_f> (&scdc_dataset_webdav2_access::do_cmd_rm));

        //dataset_cmds_add("get_buffered", static_cast<dataset_cmds_do_cmd_f> (&scdc_dataset_webdav2_access::do_cmd_get_buffered));
        dataset_cmds_add("lseek", static_cast<dataset_cmds_do_cmd_f> (&scdc_dataset_webdav2_access::do_cmd_lseek));
        dataset_cmds_add("open", static_cast<dataset_cmds_do_cmd_f> (&scdc_dataset_webdav2_access::do_cmd_open));
        dataset_cmds_add("close", static_cast<dataset_cmds_do_cmd_f> (&scdc_dataset_webdav2_access::do_cmd_close));
...
bool scdc_dataprov_webdav2_access::close(scdc_result &result) {
...
scdc_dataset * scdc_dataprov_webdav2_access::dataset_open(std::string &path, scdc_result &result) {

    SCDC_TRACE("dataset_open: path: '" << path << "'");

    scdc_dataset_webdav2_access * dataset_webdav = new scdc_dataset_webdav2_access(this);

    //TODO set this to something usefull (if the server is reacheable is alreday checked in scdc_dataprov_webdav2_access::scdc_dataprov_webdav2_access())
...
bool scdc_dataprov_webdav2_access::dataset_close(scdc_dataset *dataset, scdc_result &result) {
