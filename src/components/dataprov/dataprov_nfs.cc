...
class scdc_dataset_nfs_store: public scdc_dataset
{
  private:
    ...
    {
      scdc_dataprov_nfs_store *dataprov_nfs = static_cast<scdc_dataprov_nfs_store *>(dataprov);
    ...
    bool _rm_recursive(const char *const path, scdc_dataset_output_t *output)
    {
      scdc_dataprov_nfs_store *dataprov_nfs = static_cast<scdc_dataprov_nfs_store *>(dataprov);
      ...
      {
        SCDC_FAIL("_rm_recursive: nfs_opendir('" << path << "') failed: "
          << dataprov_nfs->_libnfs_err());

        SCDC_DATASET_OUTPUT_PRINTF(output, "entries of store cannot be "
          "enumerated");
      ...
      while ((entry = nfs_readdir(dataprov_nfs->_nfs_context, dir)))
      {
        ...
        if (entry->type == NF3DIR)
        {
          ...
          if (!_rm_recursive(recursive_path.str().c_str(), output))
          {
        ...
        else
        {
          ...
          {
            SCDC_FAIL("_rm_recursive: nfs_unlink('" << file_path_str << "') "
              "failed: " << dataprov_nfs->_libnfs_err());

            SCDC_DATASET_OUTPUT_PRINTF(output, "cannot delete file %s",
              file_path_str.c_str());
      ...
      {
        SCDC_FAIL("_rm_recursive: nfs_rmdir('" << path << "') failed: "
          << dataprov_nfs->_libnfs_err());

        SCDC_DATASET_OUTPUT_PRINTF(output, "empty directory cannot be deleted");
  ...
  public:
    scdc_dataset_nfs_store(scdc_dataprov_nfs_store *dataprov_)
    ...
    bool do_cmd_info(const std::string &params, scdc_dataset_input_t *input, scdc_dataset_output_t *output)
    {
      ...
      if (nfs_ret != 0)
      {
        SCDC_FAIL("do_cmd_info: nfs_opendir('/') failed: "
          << dataprov_nfs->_libnfs_err());

        SCDC_DATASET_OUTPUT_PRINTF(output, "stores cannot be enumerated");
      ...
      SCDC_DATASET_OUTPUT_PRINTF(output, "admin: %s, stores: %" scdcint_fmt,
        admin_mode ? "yes" : "no", stores);
    ...
    bool do_cmd_ls(const std::string &params, scdc_dataset_input_t *input, scdc_dataset_output_t *output)
    {
      ...
      if (admin_mode || pwd.size() == 0)
      {
        ...
        {
          SCDC_FAIL("do_cmd_ls: nfs_opendir('/') failed: "
            << dataprov_nfs->_libnfs_err());
        ...
        while ((entry = nfs_readdir(dataprov_nfs->_nfs_context, dir)))
        {
          ...
          SCDC_DATASET_OUTPUT_PRINTF_APPEND(output, "%s|", entry->name);
        }
      }
      ...
      else
      {
        ...
        if (nfs_opendir(dataprov_nfs->_nfs_context, libnfs_path.str().c_str(),
          &dir) != 0)
        {
          SCDC_FAIL("do_cmd_ls: nfs_opendir('" << libnfs_path.str() << "') "
            "failed: " << dataprov_nfs->_libnfs_err());
        ...
        while ((entry = nfs_readdir(dataprov_nfs->_nfs_context, dir)))
        {
          ...
          SCDC_DATASET_OUTPUT_PRINTF_APPEND(output, "%s:%s:%llu|", entry->name,
    ...
    bool do_cmd_put(const std::string &params, scdc_dataset_input_t *input, scdc_dataset_output_t *output)
    {
      ...
      if (params.size() == 0)
      {
        SCDC_DATASET_OUTPUT_PRINTF(output, "no filename specified");
      ...
      if (admin_mode)
      {
        ...
        if (libnfs_ret != 0)
        {
          ...
          /* Any other error */
          SCDC_FAIL("do_cmd_put: nfs_mkdir('" << libnfs_path.str() << "') "
            "failed: " << dataprov_nfs->_libnfs_err());
        ...
        if (libnfs_ret != 0)
        {
          SCDC_FAIL("do_cmd_put: nfs_chmod('" << libnfs_path.str() << "') "
            "to 777 failed: " << dataprov_nfs->_libnfs_err());

          SCDC_DATASET_OUTPUT_PRINTF(output, "changing new store permissions "
            " failed");
      ...
      if (pwd.size() == 0)
      {
        SCDC_DATASET_OUTPUT_PRINTF(output, "no store specified");
      ...
      else
      {
        ...
        if (libnfs_ret == -ENOENT)
        {
          ...
          if (libnfs_ret != 0)
          {
            ...
            SCDC_DATASET_OUTPUT_PRINTF(output, "entry can not be created: %s", dataprov_nfs->_libnfs_err());
        ...
        } else if (libnfs_ret != 0)  /* Any other error; fail completely. */
        {
          ...
          SCDC_DATASET_OUTPUT_PRINTF(output, "entry can not be accessed: %s", dataprov_nfs->_libnfs_err());
        ...
        do {
          ...
          if (n < 0)
          {
            ...
            SCDC_DATASET_OUTPUT_PRINTF(output, "writing failed: %s", dataprov_nfs->_libnfs_err());

            break;
          }

        } while (input->next && input->next(input) == SCDC_SUCCESS);
    ...
    bool do_cmd_get(const std::string &params, scdc_dataset_input_t *input, scdc_dataset_output_t *output)
    {
      ...
      else
      {
        ...
        if (!output)
        {
          ...
          SCDC_DATASET_OUTPUT_PRINTF(output, "no output available");
        ...
        if (nfs_stat64(dataprov_nfs->_nfs_context, dest_path_str.c_str(),
          &st) != 0)
        {
          SCDC_FAIL("do_cmd_get: nfs_stat64('" << dest_path_str << "') "
            "failed: " << dataprov_nfs->_libnfs_err());

          SCDC_DATASET_OUTPUT_PRINTF(output, "entry does not exist or can not "
            "be accessed");
        ...
        if (!output->next(output))
        {
          ...
          SCDC_DATASET_OUTPUT_PRINTF(output, "getting output failed");
    ...
    bool do_cmd_rm(const std::string &params, scdc_dataset_input_t *input, scdc_dataset_output_t *output)
    {
      ...
      if (admin_mode || pwd.size() == 0)
      {
        ...
        /* Perform a recursive removal of the directory corresponding to the
           ...
           check against.) */
        if (_rm_recursive(libnfs_path.str().c_str(), output))
        {
          SCDC_DATASET_OUTPUT_CLEAR(output);
      ...
      else
      {
        ...
        if (nfs_stat64(dataprov_nfs->_nfs_context, libnfs_path_str.c_str(),
          &st) != 0)
        {
          SCDC_FAIL("do_cmd_rm: nfs_stat64('" << libnfs_path_str << "') "
            "failed: " << dataprov_nfs->_libnfs_err());
        ...
        {
          SCDC_DATASET_OUTPUT_PRINTF(output, "entry is a directory and cannot "
            "be removed");
        ...
        {
          SCDC_FAIL("do_cmd_rm: nfs_unlink('" << libnfs_path_str << "') "
            "failed: " << dataprov_nfs->_libnfs_err());

          SCDC_DATASET_OUTPUT_PRINTF(output, "entry cannot be deleted");
...
scdc_dataset *scdc_dataprov_nfs_store::dataset_open(const char *path, scdcint_t path_size, scdc_dataset_output_t *output)
{
  ...
  if (path && !dataset_nfs->do_cmd_cd(std::string(path, path_size).c_str(), NULL, output))
  {
...
const char *scdc_dataprov_nfs_store::_libnfs_err(void)
{
