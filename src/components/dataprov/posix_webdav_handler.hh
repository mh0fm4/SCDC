...
#ifndef __POSIX_WEBDAV_HANDLER_HH__
#define __POSIX_WEBDAV_HANDLER_HH__
...
class scdc_posix_webdav_handler
{
  public:
    static const char * const type;

    typedef DAVIX_DIR *dir_t;
    static const dir_t dir_null;

    typedef DAVIX_FD *file_t;
    static const file_t file_null;

    std::string last_result;

    scdc_posix_webdav_handler():no100continue(false), davix_posix(&davix_context) {};
  ...
  private:
    std::string root_url, username, password;
    bool no100continue;

    Davix::Context davix_context;
    Davix::DavPosix davix_posix;
    Davix::RequestParams davix_request_params;

    void set_last_result(Davix::DavixError* daverr);
};


#endif /* __POSIX_WEBDAV_HANDLER_HH__ */
