...
typedef void *repoH_dataprov_t;
typedef void *repoH_dataset_t;
...
void *repoH_open(const char *conf, va_list ap)
{
  ...
  printf("repoH_open: conf: '%s', args: '%p'\n", conf, param);

  return (repoH_dataprov_t) 0xDEADBEEF;
}


scdcint_t repoH_close(void *dataprov)
{
  printf("repoH_close: dataprov: '%p'\n", dataprov);
...
scdcint_t repoH_config(void *dataprov, const char *cmd, const char *param, const char *input, scdcint_t input_size, char **output, scdcint_t *output_size)
{
  ...
  {
    printf("repoH_config: listing parameters\n");
  ...
  {
    printf("repoH_config: setting '%s' to '%.*s'\n", param, (int) *output_size, *output);

  } else if (strncmp(cmd, "get", 3) == 0)
  {
    printf("repoH_config: getting '%s'\n", param);
...
{
  repoH_dataset_t *dataset_repoH;
  
  union { void *dataprov; repoH_dataset_t dataset_repoH; } z;


  printf("repoH_dataset_open: dataprov: '%p', path: '%s'\n", dataprov, path);

  dataset_repoH = (repoH_dataset_t *) malloc(sizeof(repoH_dataset_t));

  z.dataprov = dataprov;

  *dataset_repoH = ((char *) z.dataset_repoH) + 1;

  printf("repoH_dataset_open: dataprov: return '%p' (%p)\n", dataset_repoH, *dataset_repoH);

  return dataset_repoH;
}


scdcint_t repoH_dataset_close(void *dataprov, void *dataset)
{
  repoH_dataset_t *dataset_repoH = (repoH_dataset_t *) dataset;


  printf("repoH_dataset_close: dataprov: '%p', dataset: '%p' (%p)\n", dataprov, dataset_repoH, *dataset_repoH);

  free(dataset_repoH);

  printf("repoH_dataset_close: dataprov: return '%" scdcint_fmt "'\n", SCDC_SUCCESS);
...
void *repoH_dataset_open_read_state(void *dataprov, const char *buf, scdcint_t buf_size)
{
  ...
  printf("repoH_dataset_open_read_state: dataprov: '%p', buf: '%.*s'\n", dataprov, (int) buf_size, buf);

  dataset_repoH = (repoH_dataset_t *) malloc(sizeof(repoH_dataset_t));

  z_snscanf(buf, buf_size, "%p", dataset_repoH);

  printf("repoH_dataset_open_read_state: dataset: '%p' (%p)\n", dataset_repoH, *dataset_repoH);
...
scdcint_t repoH_dataset_close_write_state(void *dataprov, void *dataset, char *buf, scdcint_t buf_size)
{
  ...
  printf("repoH_dataset_close_write_state: dataset: '%p' (%p)\n", dataset_repoH, *dataset_repoH);

  n = snprintf(buf, buf_size, "%p", *dataset_repoH);

  free(dataset_repoH);

  printf("repoH_dataset_close_write_state: return: '%" scdcint_fmt "', buf: '%.*s'\n", n, (int) buf_size, buf);
...
scdcint_t repoH_dataset_get_next(scdc_dataset_output_t *output)
{
  ptrdiff_t x;


  printf("repoH_dataset_get_next: output: '%p'\n", output);
  print_dataset_output("repoH_dataset_get_next: ", output);
  ...
  output->next = (x > 0)?repoH_dataset_get_next:NULL;
  output->data = (void *) x;
...
scdcint_t repoH_dataset_cmd(void *dataprov, void *dataset, const char *cmd, const char *params, scdc_dataset_input_t *input, scdc_dataset_output_t *output)
{
  ...
  printf("repoH_dataset_cmd: dataprov: '%p', dataset: '%p', cmd: '%s', params: '%s', input: '%p', output: '%p'\n", dataprov, dataset, cmd, params, input, output);
  print_dataset_input("repoH_dataset_cmd: ", input);
  print_dataset_output("repoH_dataset_cmd: ", output);
  ...
  if (strncmp(cmd, "get", 3) == 0)
  {
    ...
    } else if (strncmp(params, "in5out", 6) == 0)
    {
      ...
      if (input)
      {
        scdc_dataset_input_redirect(input, "to:file", "repoH_get.tmp");

        scdc_dataset_output_redirect(output, "from:file", "repoH_get.tmp");
      }
